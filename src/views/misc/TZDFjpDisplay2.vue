<template>
  ...
  <div class="fj_wrap" v-bind:style="{ backgroundImage: bgImg}">
...
" v-if="(index%5==0)">
              <table>
                <thead>
                <tr>
                  <th v-for="item in tableFiled" :key="item.name
...
">{{item.title}}
                ...
                <tbody>
                <tr v-for="(item,n) in tableData" :key="item.id

" v-if="n>=index&&n<5+index">
                  <td v-for="fileditem in  tableFiled">{{tableData[n][fileditem.name
    ...
    <el-row class="fj_foot"
      ...
      <el-row class="fj_foot_top">

        <el-col :span="12" class="fj_foot_top_name"
                v-bind:style="{ fontSize: fontHotelNameSize,color:fontHotelNameColor,fontFamily:fontHotelNameFamily}">
          <div>
            {{hotelName}}
          </div>
        </el-col>
        <el-col :span="12" class="fj_foot_top_weather"
      ...
      <el-row class="fj_foot_bottom">
        ...
        <el-col :span="6" class="fj_foot_bottom_time"
...
//                                    + "<br/>" + weather_data[n].weather
            ...
            ul.innerHTML = "  <div class='el-col el-col-6' style='text-align: center;'>" + city + "</div>" + strli;
...
<style>
  ...
  /*table   房型房价的table设置 本页只有一个table  */
